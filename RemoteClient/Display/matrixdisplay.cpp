    ...
    ui(new Ui::MatrixDisplay)
{
    ...
    connect(ui->tBtnAlarm,&QToolButton::clicked,this,&MatrixDisplay::slotBtnAlarmInfo);
    connect(ui->tBtnError,&QToolButton::clicked,this,&MatrixDisplay::slotBtnErrorInfo);
    connect(ui->tBtnHead,&QToolButton::clicked,this,&MatrixDisplay::slotBtnHeadPage);
    connect(ui->tBtnTail,&QToolButton::clicked,this,&MatrixDisplay::slotBtnTailPage);
    connect(ui->tBtnLast,&QToolButton::clicked,this,&MatrixDisplay::slotBtnLastPage);
    connect(ui->tBtnNest,&QToolButton::clicked,this,&MatrixDisplay::slotBtnNestPage);


    connect(ui->tBtnMute,&QToolButton::clicked,this,&MatrixDisplay::slotBtnMute);
    connect(ui->tBtnReset,&QToolButton::clicked,this,&MatrixDisplay::slotBtnReset);
    connect(this,SIGNAL(sigSetValue(int)),ui->scrollArea->verticalScrollBar(),SLOT(setValue(int)));

    m_timer = new QTimer;
    connect(m_timer,&QTimer::timeout,this,&MatrixDisplay::slotTimeOut);
    m_timer->start(300);

}

bool MatrixDisplay::eventFilter(QObject *watched, QEvent *event)
{
...
void MatrixDisplay::setNodeCount(int nodeCount)
{
...
void MatrixDisplay::confMatrixBtn()
{
...
int MatrixDisplay::getPageCount(int nodeCount)
{
...
QStringList MatrixDisplay::addStringList(int pass, int canId, int type, int sts, int value, QString alarmTime)
{
...
void MatrixDisplay::delStringList(QList<QStringList> stringList, int pass, int canId, int state)
{
    QString passStr = QString::number(pass);
    QString canIdStr= QString::number(canId);
    QString stsStr;
...
void MatrixDisplay::confHostAddr(QString host)
{
    m_hostAddr = host;
}


QString MatrixDisplay::modType(int type)
{
    QString typeStr;
    switch (type) {
    case LEAK:
...
void MatrixDisplay::showCurNodeValue(int index)
{
...
void MatrixDisplay::showNodeValue(int pass, int canId)
{
...
void MatrixDisplay::slotBtnClick(int index)
{
    dataClean();
    //文本长度
...
void MatrixDisplay::slotBtnAlarmInfo()
{
...
void MatrixDisplay::slotBtnErrorInfo()
{
    InfoHintDlg infoHintDlg(this);
...
void MatrixDisplay::slotTimeOut()
{
...
void MatrixDisplay::slotBtnMute()
{
    if(QMessageBox::question(this,tr("静音操作"),tr("你确定要主机静音吗?"),tr("取消"),tr("确定")))
        emit sigMute();
}

void MatrixDisplay::slotBtnReset()
{
...
void MatrixDisplay::slotNodeUpdate(QByteArray byteArray)
{
    m_connTimes = 0;
    quint16 index = 0;
...
void MatrixDisplay::slotBtnHeadPage()
{
    m_curPage = 0;
    emit sigSetValue(0);
    ui->tBtnLast->setEnabled(false);
...
void MatrixDisplay::slotBtnTailPage()
{
...
void MatrixDisplay::slotBtnLastPage()
{
    m_curPage--;
    if (m_curPage > 0) {
        emit sigSetValue(m_curPage*PAGESIZE);
...
void MatrixDisplay::slotBtnNestPage()
{
    m_curPage++;
